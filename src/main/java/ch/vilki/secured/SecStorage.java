...
public class SecStorage {
    ...
    public static String ENC_VERSION="STORAGE@@ENC_VERSION";
        ...
        if (windPapiSupported) {
           logger.info("Using windows encryption to secure storage");
            try {
                ...
                String saltedHash = new Enc().getSaltedHash(masterPassword.get_value());
    ...
    public static void createNewSecureStorageFromCSVFile(String csvFile,String labelAttribute, String labelKey, String[] attributesEncrypted, SecureString masterPassword, String storageFile) throws Exception {
    ...
    public static boolean isSecuredWithCurrentUser(String fileName) throws Exception {
    ...
    public static boolean isSecured(String fileName) throws SecureStorageException, IOException {
        _storage = new SecStorage();
        _storage._secureProperties = SecureProperties.openSecuredProperties(fileName,true);
        return _storage.hasProperty(MASTER_PASSWORD_HASH);
    }

    public static boolean isWindowsSecured(String fileName) throws SecureStorageException, IOException {
        _storage = new SecStorage();
        _storage._secureProperties = SecureProperties.openSecuredProperties(fileName, true);
    ...
    public static SecStorage recryptSecuredStorage(String fileName, SecureString masterPassword) throws Exception {
        if(masterPassword == null)
        {
            throw new SecureStorageException(SecureStorageException.MASTER_KEY_NOT_SET);
        }
        _storage = new SecStorage();
        _storage._secureProperties = SecureProperties.openSecuredProperties(fileName, false);

        if(_storage.getPropStringValue(ENC_VERSION) == null || !_storage.getPropStringValue(ENC_VERSION).equalsIgnoreCase("2")){
            logger.info("found old version, recrypt all with new encryption");
            SecureProperties securedProperties = SecureProperties.openSecuredProperties(fileName,false);
            String master_key_hash = securedProperties.getStringProperty(MASTER_PASSWORD_HASH);
            if(master_key_hash != null)
            {
                if(!Password.check(masterPassword,new SecureString(master_key_hash))) {
                    throw new SecureStorageException(SecureStorageException.PASSWORD_NOT_CORRECT);
                }
            }
            else {
                throw new SecureStorageException("can not check password, master key hash not set");
            }
            SecureProperty s1 = securedProperties.getProperty("MASTER_PASSWORD_WINDOWS_SECURED");
            System.out.println(s1);


            String newFile = Paths.get(fileName).getParent().toAbsolutePath().toString() + "//" + Paths.get(fileName).getFileName().toString().replace(".properties","") + "_recrypted";
             SecStorage.createNewSecureStorage(newFile,masterPassword,true);
            SecStorage newSecuredStorage = SecStorage.open_SecuredStorage(newFile+".properties",masterPassword);
            securedProperties.getAllKeys().forEach(key->{
                SecureProperty prop = securedProperties.getProperty(key);
                if(prop.is_encrypted()){
                    String encrypted = prop.get_value();
                    SecureString decrypted = Password.decrypt(encrypted,masterPassword);

                    if(decrypted==null){
                        logger.error("could not add property:" + key);
                    }
                    else{
                        try {
                            newSecuredStorage.addSecuredProperty(key,decrypted);
                        } catch (SecureStorageException e) {
                            logger.error("could not add property:" + key,e);
                        }
                    }
                    ...
                    try {
                        newSecuredStorage.addUnsecuredProperty(prop.get_key(),prop.get_value());
                    } catch (SecureStorageException e) {
                        logger.error("could not add property:" + prop.get_key(),e);
                    }
                }

            });
            return newSecuredStorage;
        }
        else{
            return open_SecuredStorage(fileName,masterPassword);
        }
    }

    public static SecStorage open_SecuredStorage(String fileName, SecureString masterPassword) throws Exception {
        if(masterPassword == null)
        {
            throw new SecureStorageException(SecureStorageException.MASTER_KEY_NOT_SET);
        }
    ...
    public static void changeMasterPassword(String fileName, SecureString currentPassword, SecureString newPassword) throws Exception {
    ...
    public static void secureWithCurrentUser(String fileName, SecureString masterPassword) throws Exception {
        ...
        _storage._winDPAPI = WinDPAPI.newInstance(WinDPAPI.CryptProtectFlag.CRYPTPROTECT_UI_FORBIDDEN);
        byte[] cipherTextBytes = _storage._winDPAPI.protectData(masterPassword.getBytes(StandardCharsets.UTF_8));
    ...
    public boolean checkMasterKey(SecureString masterPassword) {
        ...
        {
            try {
                if(!new Enc().check(masterPassword,master_key_hash)) return false;
                return true;
            } catch (Exception e) {
    ...
    private void addWindowsCheck(SecureString masterPassword) throws Exception {
        String saltedHash = new Enc().getSaltedHash(masterPassword.get_value());
        byte[] cipherTextBytes = _storage._winDPAPI.protectData(masterPassword.getBytes(StandardCharsets.UTF_8));
    ...
    private String protect(SecureString secureString)  {
        ...
        try {
             encrypted = new Enc().encrypt(secureString.toString(),masterPassword.toString());
        } catch (Exception e) {
    ...
    private SecureString unprotect(String protectedValue){
        ...
        if(_winDPAPI == null && _masterPassword != null)
        {
            ...
            try {
                ...
                GuiHelper.ERROR_DETAILED("Error Decrypting",e.toString(),e.fillInStackTrace().getMessage());
        ...
        else if(_winDPAPI != null && _masterPassword == null)
        {
            ...
            SecureString decrypted = null;
            try {
                 decrypted = new SecureString(new Enc().decrypt(protectedValue,masterPassword.toString()));
            } catch (Exception e) {
                logger.error("Error decrypting",e);
            }
            masterPassword.destroyValue();
